...
const FunctionBody: React.FC = () => {
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
      ...
      />
      <p>이 기능을 사용하면 새 멤버가 들어올 때마다 카미봇이 환영 메세지를 보내드려요! <br />
        아래 옵션을 사용해 환영 메세지를 설정할 수 있어요.</p>
      <ControllableStates
      ...
      {mode === 'manual' ? (
        ...
        <>
          <Typography variant="caption" color="textSecondary">지금은 사용자 지정 프롬프트를 수정할 수 없어요. 하지만 걱정 마세요! 이미 가장 완벽한 프롬프트가 적용되어 있어요.</Typography>
          <TextField
            label="환영 메세지 생성 프롬프트"
            multiline
            rows={4}
            value={"{{새_멤버_별명}}님이 {{서버_이름}} 서버에 새로 들어왔어요. 카미봇이 한두 줄 정도 축하 메세지를 작성해주세요!"}
            onChange={handleWelcomeMessageChange}
            disabled={true}
          />
        </>
