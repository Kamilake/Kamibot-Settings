...
import { Button, Dialog, DialogActions, DialogContent, DialogContentText, DialogTitle, IconButton, ListItem } from '@mui/material';
...
const listItemData = [
  ...
  { id: "twitter_embed", name: '트위터 미리보기', text: "트위터 링크에 미리보기 표시", icon: <Twitter />, help: "트위터(X) 링크를 누르지 않아도 동영상을 재생하거나 사진을 고화질로 보거나 제한된 게시물의 미리보기를 확인하세요! 이렇게 하면 트위터 유저와 트위터를 사용하지 않는 유저 모두 훨씬 편하게 포스트를 읽을 수 있어요.", disabled: false },
  { id: "docs_converter", name: '미디어 변환', text: "안 보이는 사진/동영상/파일을 보여주기", icon: <DocumentScanner />, help: "미디어 변환을 활성화하면 Discord에 업로드했을 때 올바르게 표시되지 않고 첨부파일로 올라가는 사진, 동영상, 문서, 파일들을 카미봇이 올바르게 변환해서 보여줄 수 있어요. 예를들어 avif, jfif, mkv, hwp 같은 확장자를 가진 파일들 말이에요!", disabled: false },
  ...
  { id: "tts_join_notify", name: '입퇴장 알림', text: "멤버 음성 채널 입장퇴장 알려주기", icon: <CampaignIcon />, help: <>화면을 보고 있지 않아도 누가 들어오고 나간 지 알 수 있도록 할 수 있어요. 누군가 들어오면 카미봇이 "Kami님 입장" 처럼 알려줄 거예요.<br></br><L to={`/settings/guild/tts${window.location.search}`}>음성 알림</L>에서 자세히 설정하거나 모든 채널에서 비활성화할 수 있어요.</>, category: 'tts', disabled: false },
  ...
  { id: "llm", name: 'AI 어시스턴트', text: "@카미봇 멘션을 사용해 어디서나 카미봇의 AI채팅을 사용하기", icon: <MarkChatReadIcon />, category: 'llm', disabled: false },
  ...
  React.useEffect(() => {
    if (!loading && data.name !== undefined) {
      ...
      setSwitchStates((prevState) => ({
        ...
        ['docs_converter']: data.use_docs_converter,
      ...
      subheader={
        <>
          ...
          <List dense sx={{ border: '1px solid rgba(0, 0, 0, 0.12)', borderRadius: '4px' }}>
            <ListItem key="always_use">
              <ListItemIcon>
                <Check color='success' />
              </ListItemIcon>
              <ListItemText primary="이 채널에서 기능 항상 사용하기" />
            </ListItem>
            <ListItem key="use_default">
              <ListItemIcon>
                <HorizontalRule />
              </ListItemIcon>
              <ListItemText primary="서버 기본값 채널 설정에 따르기"
                secondary={<>전체 설정을 바꾸시려면 <L to={`/settings/guild/general${window.location.search}`}>채널 기본값 설정</L>을 사용하세요</>} />
            </ListItem>
            <ListItem key="never_use">
              <ListItemIcon>
